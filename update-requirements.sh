...
sed -i "s/pycups/pycups<2/" requirements-latest.txt
...
cat requirements3-*.txt | grep -v "sys_platform == 'win32'" | sed -r -e 's,^([A-Za-z0-9_\.-]+)(.*)$,\1,' | tr A-Z a-z | sort | uniq | grep -v suds-jurko > requirements3-latest.txt
